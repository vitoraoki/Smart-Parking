...
import kotlinx.android.synthetic.main.fragment_u_events_list.*
 ...
 * Use the [UEventsListFragment.newInstance] factory method to
 * create an instance of this fragment.
 */
class UEventsListFragment : Fragment() {
    // TODO: Rename and change types of parameters
    private var param1: String? = null
    private var param2: String? = null

    private val repetitive = 0
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_u_events_list, container, false)
    }
    ...
    private fun showAllEvents(repetitive: Int) {
        ...
        uniEventsList.adapter = eventAdapter

        uniEventsList.setOnItemClickListener { parent, view, position, id ->
            val event = eventAdapter.getItem(position)
            if (event != null) {
                sngEventItemClick(event)
            }
        }

        // Open a dialog when long click on event item, to confirm its delete or not
        uniEventsList.setOnItemLongClickListener { parent, view, position, id ->
            val event = eventAdapter.getItem(position)
            if (event != null) {
    ...
    private fun sngEventItemClick(event: Event) {
        ...
        val alertDialogBuilder = AlertDialog.Builder(requireContext())
            .setTitle("Deseja acessar o evento?")
            .setMessage("Caso o evento seja acessado, ele será deletado após ser finalizado")

        // Deal with confirm button to access the event
        ...
        // Deal with dismiss button to not access the event
